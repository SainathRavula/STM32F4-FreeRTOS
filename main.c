...
StackType_t fpuTaskStack[FPU_TASK_STACK_SIZE] CCM_RAM;
// Declares an array named fpuTaskStack of type StackType_t  
// Put task stack in CCM

StaticTask_t fpuTaskBuffer CCM_RAM;  
// Put TCB in CCM

void init_USART3(void);
void test_FPU_test(void* pvParameters);
...
/* 
configUSE_STATIC_ALLOCATION and configUSE_TIMERS are both set to 1, so the application must provide an implementation of vApplicationGetTimerTaskMemory() to provide the memory that is used by the Timer service task. 
*/
